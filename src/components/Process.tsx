  ...
  useEffect(() => {
    ...
    const onScroll = () => {
      ...
      raf = requestAnimationFrame(() => {
        ...
        const startPct = 2;
        const endPct = -78;
        const x = reduce ? startPct : startPct + (endPct - startPct) * progress;
    ...
    <section id="process" className="bg-paper text-ink">
      ...
      <div ref={ref} className="relative hidden h-[420vh] lg:block">
        <div className="sticky top-0 flex h-screen flex-col overflow-hidden">
          <div className="mx-auto flex w-full max-w-[1600px] items-end justify-between px-[var(--spacing-gutter)] pt-28">
            <div>
          ...
          <div
            ref={trackRef}
            className="mt-12 flex gap-8 px-[var(--spacing-gutter)] will-change-transform"
          >
            {process.map((p) => (
              <article
                key={p.index}
                className="group relative h-[54vh] w-[42vw] shrink-0 overflow-hidden rounded-sm bg-graphite"
              >
                <Image
                  src={p.image}
                  alt={p.title}
      ...
      <div className="px-[var(--spacing-gutter)] py-20 lg:hidden">
        ...
        <ol className="mt-10 space-y-5">
          {process.map((p) => (
            <li key={p.index} className="relative overflow-hidden rounded-sm bg-graphite">
